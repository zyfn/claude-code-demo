...
- Rate limit handling
- Model fallback
- Error classification (rate limit, model unavailable, etc.)

Note: max_output_tokens handling is done directly in agent_loop (not here),
since agent_loop has the full context (accumulated text, tool calls) needed
to decide between escalation vs recovery.
"""
...
async def with_retry(
    ...
    for attempt in range(1, config.max_retries + 2):
        try:
            async for chunk in call_model(messages, tools, max_tokens):
                yield chunk
            return  # Success
...
def _handle_prompt_too_long(
    ...
    # Prompt too long is not recoverable via retry — the caller must
    # compact context before retrying. Signal failure so the loop exits.
    return RetryAction(action="fail", message="Prompt too long")
