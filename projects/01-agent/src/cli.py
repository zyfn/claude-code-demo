"""CLI session — TUI loop, slash command dispatch, event forwarding.

Mirrors Claude Code's cli.tsx: handles the interactive session loop,
slash commands (/skill), and event rendering.
"""
...
from src.agent.query.types import TextEvent, ToolStartEvent, ToolResultEvent, FinalEvent, TurnStart
from src.agent.query.retry import RetryConfig
from src.tools import get_all_tools
from src.ui.tui import TUI
from src.ui.project import ProjectContext
from src.agent.prompt import build_system_prompt
from src.context.user import load_claude_md
...
async def cli_loop():
    ...
    # Inject CLAUDE.md as user message at session start
    claude_md = load_claude_md(settings.project_root)
    if claude_md:
        agent.inject_messages(
            [Message(role="user", content=f"## CLAUDE.md\n\n{claude_md}")]
        )
