...
class _LoopState:
    ...
    def complete(self, reason: str, **kwargs) -> QueryComplete:
        return QueryComplete(reason=reason, turn_count=self.turn, messages=self.messages, **kwargs)
...
async def query(
    ...
    while state.turn < max_turns:
        ...
        if token_count >= deps.context_limit - BLOCKING_BUFFER:
            yield state.complete("blocking_limit")
            return

        # ── Stream LLM ────────────────────────────────────────────────────

        if _is_aborted(deps.abort_signal):
            yield state.complete("aborted")
            return
        ...
        try:
            ...
            yield state.complete("model_error", error=str(e))
            return
        except PromptTooLongError:
            yield state.complete("prompt_too_long")
            return
        ...
        if not acc.tool_calls:
            transition = await _handle_text_response(acc, state, deps)
            if isinstance(transition, QueryComplete):
                yield transition
                return
            state = transition
            continue
        ...
        # Build updated messages with current turn included (Claude Code pattern:
        # attachments see the full picture including this turn's tool calls)
        updated = [*state.messages, assistant_msg, *tool_messages]

        # Per-turn attachments (e.g. todo reminder)
        attachments = deps.get_attachments(updated) if deps.get_attachments else []

        state = replace(
            state,
            messages=[*updated, *attachments],
            max_output_override=None,
            max_output_recovery_count=0,
            transition="next_turn",
        )

    yield state.complete("max_turns")
...
async def _handle_text_response(
    acc: _ResponseAcc, state: _LoopState, deps: QueryDeps,
) -> _LoopState | QueryComplete:
    """Handle LLM response with no tool calls. Returns updated state (continue) or QueryComplete (stop)."""

    # Empty response → escalate max_tokens
    if not acc.text and state.max_output_override is None:
        return replace(state, max_output_override=ESCALATED_MAX_TOKENS,
                       transition="max_output_tokens_escalate")

    # Still empty → recovery prompt
    ...
    if not acc.text:
        return state.complete("max_output_exhausted")

    # Stop hooks
    stop_results = await deps.hooks.dispatch(HookEvent.STOP, {
        "messages": state.messages, "text": acc.text,
    })
    if any(r is True for r in stop_results):
        return replace(state, transition="stop_hook_blocking")

    # Normal completion
    return QueryComplete(
        reason="completed", text=acc.text, turn_count=state.turn,
        messages=[*state.messages, Message(role="assistant", content=acc.text)],
    )
